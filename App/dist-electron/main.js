import { app, BrowserWindow, ipcMain, shell, Tray, Menu, nativeTheme, nativeImage, screen, globalShortcut } from "electron";
...
function runMigrations(d) {
  const applied = new Set(
    d.prepare("SELECT MigrationId FROM tbl_MigrationHistory").all().map((r) => r.MigrationId)
  );
  const migrations = [
...
function startTask(text, startNow, stopOtherTasks, hideParent, bringNotes, bringTags, getTags, parentEntryId) {
  ...
  if (getTags && !parentEntry) {
    const minDate = /* @__PURE__ */ new Date();
    minDate.setDate(minDate.getDate() - settingHelper.NumDaysOfDataToLoad);
    const recentTagIds = db2.prepare(
      `
...
function saveTags(taskId, tags) {
  const db2 = getDb();
  const existingTags = db2.prepare(
    `
  ...
  const allTags = db2.prepare("SELECT ID, Name FROM tbl_Tag WHERE Name IN (" + tags.map(() => "?").join(",") + ")").all(...tags);
...
function getAllTags() {
  return getDb().prepare("SELECT * FROM tbl_Tag ORDER BY Name").all().map((r) => ({ id: r.ID, name: r.Name, description: r.Description }));
}
...
function registerIpcHandlers() {
  ...
  ipcMain.handle(
    "notes:save",
    (_e, taskId, noteId, notes, rtf) => saveNote(taskId, noteId, notes, rtf)
  );
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("window:showNewEntry", () => {
    const suggestions = getAllTasks().filter((t) => t.endedOn != null).map((t) => t.task).filter((v, i, a) => a.indexOf(v) === i).slice(0, 20);
  ...
  ipcMain.handle("window:savePosition", (_e, x, y) => {
...
function triggerToggleLast() {
  ...
  if (running) {
    ...
    const last = tasks2.filter((t) => t.endedOn != null).sort((a, b) => new Date(b.endedOn).getTime() - new Date(a.endedOn).getTime())[0];
